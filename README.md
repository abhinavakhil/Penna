...
Notion-feel editing, lossless Markdown, real tables and media, slash shortcuts, a built-in proofreader,<br/>
version history, light / dark / sepia themes, RTL, and free AI. One small package for React, Vue, Angular, Svelte, Solid or plain HTML.

[![npm version](https://img.shields.io/npm/v/@abhinavakhil/penna-core?style=flat&color=7a5af0&label=npm)](https://www.npmjs.com/package/@abhinavakhil/penna-core)
...
[Playground](https://penna.vercel.app/playground) · [Docs](https://penna.vercel.app/docs) · [npm](https://www.npmjs.com/package/@abhinavakhil/penna-core) · [Angular demo](https://codesandbox.io/p/devbox/sqxfhh) · [Changelog](CHANGELOG.md) · [Issues](https://github.com/abhinavakhil/Penna/issues)
...
React, Vue and Angular wrappers and a web component for everything else. Content is stored as JSON and rendered
...
|---|:---:|:---:|:---:|:---:|
| Works in any framework | ✅ incl. Angular forms | React only | Vanilla + wrappers | jQuery-era API |
...
- **Slash commands →** 16 blocks: headings, lists, tables, images, video, embeds, callouts, code, and AI. Fuzzy search, keyboard first, with each block's hotkey shown right in the menu.
- **Shortcuts →** Drop in ready-made blocks with `/budget`, `/impact`, `/milestones` or a hotkey. Select anything and press `+` to save it as your own `/shortcut`. Everything inserted stays editable.
- **Bubble menu →** Select text and the formatting you need appears right there. Bold, links, highlight, AI, and a **Turn into** menu to change the block type.
- **Mentions and merge fields →** `@` to mention someone from your API, `{{` to add a per-reader field like `{{first_name}}`, then preview it with a real value.
...
*A second pair of eyes, a safety net, and a quiet room.*

- **Proofreader →** Spelling mistakes, wordy phrases, weak words and hard-to-read sentences get underlined as you type. One click to fix, and a live score. Nothing changes unless you accept.
- **Version history →** Autosaves pile up quietly. Name the ones that matter, see word by word what changed since, and restore in one click.
- **Comments →** Pin a thread to the exact words. Penna keeps the anchor; your app keeps the conversation.
- **Focus mode →** The current sentence stays lit, the line you're on stays mid-screen, and the toolbar fades while you type.
- **Readability →** `readability(text)` gives a reading age, so you can nudge people toward plain language.
...
- **Preview before insert →** Responses stream into a preview, and rewrites show a word-by-word diff. Accept, retry or discard. Nothing touches the document until you say so.
...
- **Light, dark and sepia →** On `auto` it follows your app's `data-theme` or `.dark`, then the system. Or force one.
- **Theming →** One `penna.css`, all tokens on `.penna`. Match your brand in a line.
- **Right-to-left →** `dir: 'rtl'` mirrors lists, quotes, menus and the drag handle for Urdu, Arabic and Hebrew.
...
<details>
<summary><b>Angular</b> (native component, works with forms)</summary>
...
@Component({
  ...
  template: `<ngx-penna [(ngModel)]="html" placeholder="Write something…" />`,
})
export class AppComponent { html = '<p>Hello</p>' }
```

Add `node_modules/@abhinavakhil/penna-core/dist/penna.css` to `styles` in `angular.json`. Prefer no extra dependency? `<penna-editor>` from `penna-element` works too, with `CUSTOM_ELEMENTS_SCHEMA`.
...
import { shortcuts } from '@abhinavakhil/penna-core'

bootstrapApplication(AppComponent, { providers: [providePenna({ theme: 'auto', extensions: [shortcuts()] })] })
```

`<ngx-penna>` works with `ngModel`, `formControl` / `formControlName` (a disabled control makes it read-only), or `[(value)]`, and is safe for SSR. See [`packages/angular/README.md`](packages/angular/README.md).
...
| [`@abhinavakhil/penna-angular`](https://www.npmjs.com/package/@abhinavakhil/penna-angular) | [![npm](https://img.shields.io/npm/v/@abhinavakhil/penna-angular?style=flat-square&label=)](https://www.npmjs.com/package/@abhinavakhil/penna-angular) | Angular component: ngModel, reactive forms, SSR-safe |
...
pnpm --filter angular-demo dev   # Angular demo on http://localhost:4310 (build packages first)
pnpm test
pnpm typecheck
pnpm build
```
